...
int main(int argc, char *argv[]) {
    ...
    int minTemp = minTempHistory(array);
    printf("\n\nminTemp: %d\n\n", minTemp);

    int maxTempsRgisteredPerYear[YEARS];
    maxTempPerYear(array, maxTempsRgisteredPerYear);

    char *monthsOfMaxPrec[YEARS];
    monthOfMaxPrecPerYear(array, monthsOfMaxPrec);

    for (unsigned int year = 0; year < YEARS; year++) {
        printf("year: %u \t maxTemp: %d \t month MaxPrec: %s\n", 
            year + FST_YEAR,   
            maxTempsRgisteredPerYear[year], 
            monthsOfMaxPrec[year]
        );
    }
